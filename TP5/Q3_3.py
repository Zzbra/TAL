with open("dico_nom_propre.txt") as f:
    dic_content = f.readlines()

np_dict = dict()

for line in dic_content:
    np_dict[line.split("\t")[0]] = line.split("\t")[1]
...
for i in range(len(content)):
    ...
    if len(input_line) > 2:
        if input_line[4] == "hyp\n":
            ...
            dict_entry = "O"
            if input_line[1] in np_dict:
                dict_entry = np_dict[input_line[1]]
            output_line += str(i) + " , " + context[0] + " , " \
                           + input_line[1] + " , " + context[1] + " , " + dict_entry + " , " + input_line[3] + " ."
